...
bool file_replace_tokens(const string &name, const string &token, const string &new_token) {
    ...
    return executef("sed -i s/%s/%s/g %s", esc_token.c_str(), esc_new_token.c_str(),
                    esc_name.c_str())
           == 0;
}
...
run_checker_result run_checker(runp::config rpc, const string &input_file_name,
                               const string &output_file_name, const string &answer_file_name) {
    ...
    rpc.rest_args = {fs::canonical(input_file_name), fs::canonical(output_file_name),
...
runp::result run_submission_program(runp::config rpc, const string &input_file_name,
...
run_simple_interaction_result run_simple_interaction(const string &input_file_name,
...
run_compiler_result compile_with_implementer(const string &name,
    ...
    runp::config rpc(main_path / "run" / "compile", {"--custom", main_path / "run" / "runtime",
                                                     "--impl", implementer, "--lang", lang, name});
    rpc.limits = RL_COMPILER_DEFAULT;
    return run_compiler(rpc);
}
...
runp::config TestPoint::search_program(const initializer_list<string> &ids) {
    ...
    throw jgf_error("PROGNF2",
                    "Program " + list_to_string(ids) + " Not Found");  // program not found
}
...
bool TestPoint::add_info_block(const string &fkey) {
    if (ftitle.count(fkey) && ftitle[fkey] != "disabled") {
        po.li.push_back(
            InfoBlock::from_file_with_size(ftitle[fkey], fname[fkey], get_fpreview_size(fkey)));
    }
    return true;
}
...
bool TestPoint::preprocess_input(const string &input) {
    return validate(input) && check() && encode(input) && check() && encrypt(input) && check();
}
...
bool TestPoint::encode(const string &input) {
    run_transformer_result ret = transform_file(search_program({input + "encoder", "encoder"}),
...
bool TestPoint::encrypt(const string &input) {
    run_transformer_result ret = transform_file(search_program({input + "encrypter", "encrypter"}),
...
bool TestPoint::decrypt(const string &output) {
    run_transformer_result ret = transform_file(search_program({output + "decrypter", "decrypter"}),
...
bool TestPoint::decode(const string &output) {
    run_transformer_result ret = transform_file(search_program({output + "decoder", "decoder"}),
...
bool TestPoint::generate_answer(const string &input, const string &answer) {
    runp::result ret =
        run_submission_program(get_program("std"), fname["input.raw"], fname["answer.raw"]);
    if (ret.type != runp::RS_AC) {
...
bool TestPoint::grade(const string &input, const string &output, const string &answer) {
    run_checker_result ret =
        run_checker(get_program("chk"), fname[input], fname[output], fname[answer]);
    if (ret.type != runp::RS_AC) {
...
void TestPoint::complete_config() {
    map_add(fauthor,
            {{"input", AUTHOR_SETTER}, {"output", AUTHOR_USER}, {"answer", AUTHOR_SETTER}});
...
void TestPoint::complete_hack_config() {
    map_add(fauthor, {{"input", AUTHOR_USER}, {"output", AUTHOR_USER}, {"answer", AUTHOR_STD}});
...
protected:
    ...
    virtual bool _test() override {
        return prepare_io() && add_info_block("input") && encode("input") && check()
               && decode("output") && check() && grade("input", "output", "answer") && check();
    }
};
...
protected:
    ...
    virtual bool _test() override {
        return prepare_io() && add_info_block("input") && preprocess_input("input") && run()
               && postprocess_output("output") && grade("input", "output", "answer") && check();
    }

    virtual bool _hack_test() override {
        return prepare_io() && add_info_block("input") && preprocess_input("input")
               && generate_answer("input", "answer") && postprocess_output("answer") && run()
               && postprocess_output("output") && grade("input", "output", "answer") && check();
    }
};
...
protected:
    virtual void complete_config() override {
        ...
        map_add(fname, {{"program_input.raw", work_path / "io" / "raw_program_input.txt"},
    ...
    virtual bool run() {
        ...
        run_simple_interaction_result rires = run_simple_interaction(
            fname["input.raw"], fname["answer"], fname["program_input.raw"], fname["output.raw"],
            get_program("answer"), get_program("interactor"));
    ...
    virtual bool _test() override {
        return prepare_io() && add_info_block("input") && preprocess_input("input") && run()
               && decrypt("program_input") && decrypt("output") && check();
    }
    ...
    virtual void ordinary_test() {
        ...
        if (nT == 0) {  // OI
            ...
            for (int t = 1; t <= nT; t++) {
                ...
                bool skipped = false;
                for (vector<int>::iterator it = dependences.begin(); it != dependences.end();
                     it++) {
