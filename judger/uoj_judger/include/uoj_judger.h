...
/**
 * @brief given a score in a system where the full mark is 100, scale it so that the full mark
 * equals "full"
 */
...
struct PointInfo {
    ...
    PointInfo(const int &_num, const score_t &_scr, const int &_ust, const int &_usm,
              const string &_info = "default") :
    ...
    PointInfo(const int &_num, const score_t &_scr, const int &_ust, const int &_usm,
              const string &_info, const string &_in, const string &_out, const string &_res) :
        num(_num), scr(_scr), ust(_ust), usm(_usm), info(_info), in(_in), out(_out), res(_res) {
        use_li = false;
        if (info == "default") {
            if (scr == 0) {
                info = "Wrong Answer";
...
struct CustomTestInfo {
    ...
    CustomTestInfo(const int &_ust, const int &_usm, const string &_info, const string &_exp,
                   const string &_out) :
...
struct RunProgramConfig {
    ...
    string get_cmd() const {
        ...
        sout << main_path << "/run/run_program"
             ...
             << "--err=" << escapeshellarg(error_file_name) << " " << "--tl=" << limit.time << " "
             << "--ml=" << limit.memory << " " << "--ol=" << limit.output << " "
             << "--type=" << type << " " << "--work-path=" << work_path
            /*<< " " << "--show-trace-details"*/;
...
RunResult vrun_program(const char *run_program_result_file_name, const char *input_file_name,
                       const char *output_file_name, const char *error_file_name,
                       const RunLimit &limit, const vector<string> &rest) {
    ostringstream sout;
    sout << main_path << "/run/run_program"
...
RunResult run_program(const char *run_program_result_file_name, const char *input_file_name,
    ...
    va_end(vl);
    return vrun_program(run_program_result_file_name, input_file_name, output_file_name,
                        error_file_name, limit, argv);
}

RunValidatorResult run_validator(const string &input_file_name, const RunLimit &limit,
                                 const string &program_name) {
    RunResult ret = run_program(
        (string(result_path) + "/run_validator_result.txt").c_str(), input_file_name.c_str(),
        "/dev/null", (string(result_path) + "/validator_error.txt").c_str(), limit,
        ("--type=" + conf_str("val_run_type", "default")).c_str(), program_name.c_str(), NULL);
...
RunCheckerResult run_checker(const RunLimit &limit, const string &program_name,
    ...
    RunResult ret =
        run_program((result_path + "/run_checker_result.txt").c_str(), "/dev/null", "/dev/null",
                    ...
                    ("--type=" + conf_str("chk_run_type", "default")).c_str(), program_name.c_str(),
...
RunResult run_submission_program(const string &input_file_name, const string &output_file_name,
 ...
 *
 * @param prepared if it is not -1, set the internal variable as if the preparation operations have
 * been done or not (depending on prepared is non-zero or not)
 */
 ...
 * For the current version, it only moves the interactor to the work dir if it has not been done
 * yet.
 *
 * @param prepared if it is not -1, set the internal variable as if the preparation operations have
 * been done or not (depending on prepared is non-zero or not)
...
RunSimpleInteractionResult run_simple_interaction(
    ...
    const string &real_input_file_name, const string &real_output_file_name, const RunLimit &limit,
    const RunLimit &ilimit, const string &name, RunProgramConfig rpc = RunProgramConfig(),
    ...
    irpc.limit.real_time =
        rpc.limit.real_time + 1;  // one more second to prevent interactor from TLE
...
// @deprecated
// will be removed in the future
RunResult run_standard_program(const string &input_file_name, const string &output_file_name,
                               const RunLimit &limit, RunProgramConfig rpc = RunProgramConfig()) {
    prepare_run_standard_program();
    rpc.result_file_name = result_path + "/run_standard_program.txt";
    return run_submission_program(input_file_name, output_file_name, limit, "std", rpc);
}
...
run_compiler_result compile_with_implementer(const string &name,
                                             const string &implementer = "implementer") {
    string lang = conf_str(name + "_language", "auto");
    if (conf_has(name + "_unit_name")) {
        file_put_contents(work_path + "/" + name + ".unit_name", conf_str(name + "_unit_name"));
    }
    runp::config rpc(main_path + "/run/compile", {"--custom", main_path + "/run/runtime", "--impl",
...
PointInfo test_point(const string &name, const int &num, TestPointConfig tpc = TestPointConfig()) {
    ...
    if (tpc.validate_input_before_test) {
        RunValidatorResult val_ret =
            run_validator(tpc.input_file_name, conf_run_limit("validator", 0, RL_VALIDATOR_DEFAULT),
                          conf_str("validator"));
    ...
    if (!conf_is("interaction_mode", "on")) {
        ...
        if (tpc.checker == "nonempty") {
            ...
            if (usrout == "") {
                return PointInfo(num, 0, -1, -1, "default", file_preview(tpc.input_file_name),
                                 usrout, "wrong answer empty file\n");
            } else {
                return PointInfo(num, 100, -1, -1, "default", file_preview(tpc.input_file_name),
        ...
        RunSimpleInteractionResult rires = run_simple_interaction(
            tpc.input_file_name, tpc.answer_file_name, real_input_file_name, real_output_file_name,
            tpc.limit, conf_run_limit("interactor", num, RL_INTERACTOR_DEFAULT), name);

        if (rires.ires.type != runp::RS_AC) {
            return PointInfo(num, 0, -1, -1, "Interactor " + info_str(rires.ires.type),
...
PointInfo test_hack_point(const string &name, TestPointConfig tpc) {
    ...
    if (val_ret.type != runp::RS_AC) {
        ...
        return PointInfo(0, 0, -1, -1, "Invalid Input", file_preview(tpc.input_file_name), "",
                         val_ret.info);
    }
    ...
    if (!conf_is("interaction_mode", "on")) {
        ...
        if (std_ret.type != runp::RS_AC) {
            return PointInfo(0, 0, -1, -1, "Standard Program " + info_str(std_ret.type),
                             file_preview(tpc.input_file_name), "", "");
        }
        if (conf_has("token")) {
        ...
        if (rires.res.type != runp::RS_AC) {
            return PointInfo(0, 0, -1, -1, "Standard Program " + info_str(rires.res.type),
                             file_preview(real_input_file_name), "", "");
        }
    }
    ...
    if (nT == 0) {  // OI
        ...
        for (int t = 1; t <= nT; t++) {
            if (subtask_metas[t].full_score == -1) {
                subtask_metas[t].full_score =
                    remaining_subtasks_total_score / remaining_subtasks_cnt;
            }
...
template <typename TP>
bool primary_data_test(TP test_point_func,
                       const PrimaryDataTestConfig &pdtc = PrimaryDataTestConfig()) {
