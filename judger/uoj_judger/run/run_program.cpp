...
run_event next_event() {
    ...
    if (WIFEXITED(stat)) {
        if (p == 0) {
            stop_all(runp::result(
                runp::RS_JGF, "error code: ZROEX"));  // the 0th child process exited unexpectedly
        }
        e.type = ET_EXIT;
    ...
    if (WIFSIGNALED(stat)) {
        if (p == 0) {
            stop_all(
                runp::result(runp::RS_JGF,
                             "error code: ZROSIG"));  // the 0th child process signaled unexpectedly
        }
        e.type = ET_SIGNALED;
        e.sig = WTERMSIG(stat);
        return e;
    ...
    if (!WIFSTOPPED(stat)) {
        stop_all(
            runp::result(runp::RS_JGF, "error code: NSTOP"));  // expected WIFSTOPPED, but it is not
    }
            ...
            switch (e.pevent) {
                ...
                default:
                    stop_all(runp::result(runp::RS_JGF,
                                          "error code: PTRCSIG"));  // unknown ptrace signal
            }
        case SIGSTOP:
    ...
    switch (e.type) {
        ...
        case ET_EXIT:
            ...
            if (rp_children[0].flags & CPF_STARTUP) {
                stop_all(
                    runp::result(runp::RS_JGF, "error code: CPCMDER1"));  // rp_children mode error
