<?php
	...
	function dataClearProblemData($problem) {
		...
		exec("rm /var/uoj_data/prepare/$id -r");
	...
	class SyncProblemDataHandler {
		...
		private $prepare_dir, $data_dir, $tmp_dir;
		...
		private function copy_to_prepare($file_name) {
			...
			$src = escapeshellarg("{$this->prepare_dir}/$file_name");
			$dest = escapeshellarg("{$this->tmp_dir}/$file_name");
			if (isset($this->problem_extra_config['dont_use_formatter']) || !is_file("{$this->prepare_dir}/$file_name")) {
		...
		private function copy_file_to_prepare($file_name) {
			global $uojMainJudgerWorkPath;
			if (!isset($this->allow_files[$file_name]) || !is_file("{$this->prepare_dir}/$file_name")) {
				throw new UOJFileNotFoundException($file_name);
			}
		...
		private function compile_at_prepare($name, $config = array()) {
			...
			if (isset($config['path'])) {
				exec("mv {$this->tmp_dir}/$name.cpp {$this->tmp_dir}/{$config['path']}/$name.cpp");
				$work_path = "{$this->tmp_dir}/{$config['path']}";
			} else {
				$work_path = $this->tmp_dir;
			}

			$cmd_prefix = "$uojMainJudgerWorkPath/run/run_program >{$this->tmp_dir}/run_compiler_result.txt --in=/dev/null --out=stderr --err={$this->tmp_dir}/compiler_result.txt --tl=10 --ml=512 --ol=64 --type=compiler --work-path={$work_path}";
			...
			$fp = fopen("{$this->tmp_dir}/run_compiler_result.txt", "r");
			if (fscanf($fp, '%d %d %d %d', $rs, $used_time, $used_memory, $exit_code) != 4) {
				$rs = 7;
			}
			fclose($fp);
			
			unlink("{$this->tmp_dir}/run_compiler_result.txt");
			
			if ($rs != 0 || $exit_code != 0) {
				if ($rs == 0) {
					throw new Exception("<strong>$name</strong> : compile error<pre>\n" . uojFilePreview("{$this->tmp_dir}/compiler_result.txt", 100) . "\n</pre>");
				} elseif ($rs == 7) {
			...
			unlink("{$this->tmp_dir}/compiler_result.txt");
			
			if (isset($config['path'])) {
				exec("mv {$this->tmp_dir}/{$config['path']}/$name.cpp {$this->tmp_dir}/$name.cpp");
				exec("mv {$this->tmp_dir}/{$config['path']}/$name {$this->tmp_dir}/$name");
			}
		}
		private function makefile_at_prepare() {
			...
			$include_path = "$uojMainJudgerWorkPath/include";
			$cmd_prefix = "$uojMainJudgerWorkPath/run/run_program >{$this->tmp_dir}/run_makefile_result.txt --in=/dev/null --out=stderr --err={$this->tmp_dir}/makefile_result.txt --tl=10 --ml=512 --ol=64 --type=compiler --work-path={$this->tmp_dir}";
			exec("$cmd_prefix --add-readable-raw=$include_path/ /usr/bin/make INCLUDE_PATH=$include_path");
			
			$fp = fopen("{$this->tmp_dir}/run_makefile_result.txt", "r");
			if (fscanf($fp, '%d %d %d %d', $rs, $used_time, $used_memory, $exit_code) != 4) {
				$rs = 7;
			}
			fclose($fp);
			
			unlink("{$this->tmp_dir}/run_makefile_result.txt");
			
			if ($rs != 0 || $exit_code != 0) {
				if ($rs == 0) {
					throw new Exception("<strong>Makefile</strong> : compile error<pre>\n" . uojFilePreview("{$this->tmp_dir}/makefile_result.txt", 100) . "\n</pre>");
				} elseif ($rs == 7) {
			...
			unlink("{$this->tmp_dir}/makefile_result.txt");
		}
		...
		public function handle() {
			...
			$this->tmp_dir = "/var/uoj_data/tmp_$id";
			...
			try {
				...
				if (!is_file("{$this->prepare_dir}/problem.conf")) {
					throw new UOJFileNotFoundException("problem.conf");
				}

				$this->problem_conf = getUOJConf("{$this->prepare_dir}/problem.conf");
				...
				$this->allow_files = array_flip(array_filter(scandir($this->prepare_dir), function($x){return $x !== '.' && $x !== '..';}));

				$zip_file = new ZipArchive();
				if ($zip_file->open("{$this->tmp_dir}/download.zip", ZipArchive::CREATE) !== true) {
					throw new Exception("<strong>download.zip</strong> : failed to create the zip file");
				}
				
				if (isset($this->allow_files['require']) && is_dir("{$this->prepare_dir}/require")) {
					...
					if ($this->check_conf_on('submit_answer')) {
						...
						for ($num = 1; $num <= $n_tests; $num++) {
							...
							if (!isset($this->problem_extra_config['dont_download_input'])) {
								$zip_file->addFile("{$this->tmp_dir}/$input_file_name", "$input_file_name");
							}
						...
						if (!isset($this->problem_extra_config['dont_download_sample'])) {
							for ($num = 1; $num <= $n_sample_tests; $num++) {
								...
								if (!isset($this->problem_extra_config['dont_download_sample_output'])) {
									$zip_file->addFile("{$this->tmp_dir}/{$output_file_name}", "$output_file_name");
								}
							}
						}
				...
				putUOJConf("{$this->tmp_dir}/problem.conf", $this->final_problem_conf);

				if (isset($this->allow_files['download']) && is_dir("{$this->prepare_dir}/download")) {
					foreach (scandir("{$this->prepare_dir}/download") as $file_name) {
						if (is_file("{$this->prepare_dir}/download/{$file_name}")) {
							$zip_file->addFile("{$this->prepare_dir}/download/{$file_name}", $file_name);
						}
					}
				}
				...
				exec("rm {$this->tmp_dir} -r");
				return $e->getMessage();
			}

			exec("rm {$this->data_dir} -r");
			rename($this->tmp_dir, $this->data_dir);
	...
	function dataAddExtraTest($problem, $input_file_name, $output_file_name) {
		...
		$cur_dir = "/var/uoj_data/prepare/$id";
