<?php
	function handleRegisterPost() {
		...
		if (!DB::selectCount("SELECT COUNT(*) FROM user_info"))
			DB::query("insert into user_info (username, email, password, register_time, usergroup) values ('$username', '$esc_email', '$password', now(), 'S')");
		else
			DB::query("insert into user_info (username, email, password, register_time) values ('$username', '$esc_email', '$password', now())");
